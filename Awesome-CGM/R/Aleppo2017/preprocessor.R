...
file <- paste0('RawData/Testing_Data/', dataset, '/Replace-BG Dataset/Data Tables/HDeviceCGM.txt') 
...
# Filter to only include CGM records (exclude calibration records)
aleppo <- aleppo[aleppo$RecordType == "CGM", ]

# The data has days as days since study start 
# Establishing a base date to work from (default to Unix epoch)
basedate <- as.Date("1970-01-01", format = "%Y-%m-%d")
