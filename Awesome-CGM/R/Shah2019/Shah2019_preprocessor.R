...
T1DE_CGM <- read_csv("RawData/Training_Data/Shah2019/CGMND-af920dee-2d6e-4436-bc89-7a7b51239837/NonDiabDeviceCGM.csv") # CGM_reading: PtID = T1DE$id, DeviceTm = T1DE$time, Value = T1DE$gl

# Filter out calibration records
T1DE_CGM <- T1DE_CGM %>%
  filter(RecordType != "Calibration")

T1DE_screening <- read_csv("RawData/Training_Data/Shah2019/CGMND-af920dee-2d6e-4436-bc89-7a7b51239837/NonDiabScreening.csv") # PtID = T1DE$id,  Gender = NDChild$sex
T1DE_Patient<- read_csv("RawData/Training_Data/Shah2019/CGMND-af920dee-2d6e-4436-bc89-7a7b51239837/NonDiabPtRoster.csv") #AgeAsOfEnrollDt = T1DE$age
...
# For this purpose, we generate pseudo start dates, starting from January 1, 1970.
pseudo_start_date <- as.Date("1970-01-01")
...
T1DE_combined <- T1DE %>%
  ...
  # Ungroup the dataset
  ungroup() %>%
  # Select the final columns for the output dataset with original IDs and relevant fields
