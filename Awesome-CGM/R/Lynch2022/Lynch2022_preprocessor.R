...
# Read in data
data <- read_sas("RawData/Testing_Data/Lynch2022/IOBP2 RCT Public Dataset/Data Tables in SAS/iobp2devicecgm.sas7bdat")
demo <- read_sas("RawData/Testing_Data/Lynch2022/IOBP2 RCT Public Dataset/Data Tables in SAS/iobp2diabscreening.sas7bdat")
age <- read_sas("RawData/Testing_Data/Lynch2022/IOBP2 RCT Public Dataset/Data Tables in SAS/iobp2ptroster.sas7bdat")

# Select only necessary variables
data = data %>% filter(RecordType == "CGM") %>% select(PtID, DeviceDtTm, Value)
...
df_final = data %>%
  ...
  # Ensure that time is in order within subject without altering subject IDs
  arrange(id, time) %>%
  # Keep original PtID as `id` (no pseudo IDs)
